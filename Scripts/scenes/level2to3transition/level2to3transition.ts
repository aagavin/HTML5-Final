module scenes {
	export class L2toL3 extends objects.Scene {
		...
		private _nextScene: objects.Button;
		...
		public Start(): void {
			...
			this._menuLabel2 = new objects.Label(
				"You found the shark leader", "27px", "Tahoma, Geneva, sans-serif", "#ff0",
				config.Screen.HALF_WIDTH, 200
			);

			this.addChild(new objects.Label(
				"Kill the sharknado boss", "27px", "Tahoma, Geneva, sans-serif", "#ff0",
				config.Screen.HALF_WIDTH, 230
			));
			this.addChild(new objects.Label(
				"but watch out for ninja stars", "27px", "Tahoma, Geneva, sans-serif", "#ff0",
			...
			this._nextScene = new objects.Button(
				"level3Btn", 250, 400, true
			);
